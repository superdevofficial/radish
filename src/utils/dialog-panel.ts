...
export class DialogPanel implements IDialogPanel {
  ...
  async next (questionIndex?: number): Promise<any> {
  ...
  async evaluateCondition(qst: Question): Promise<any> {
    if (!qst.condition) return;
  ...
  async ask(qst: Question): Promise<any> {
    return new Promise((resolve: any) => this.cmd.ask(this.cmd.getQuestion(qst.question, qst.default), resolve));
  }

  async askWithChoices(qst: Question) {
    return new Promise((resolve) => this.cmd.askWithChoices(qst.question, qst.values, (response: any) => resolve(response)));
  }
