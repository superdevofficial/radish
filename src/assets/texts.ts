...
type RadishLocality = 'EN'|'FR';
type RadishTranslations = { [x in RadishLocality]: any };

const TEXTS: RadishTranslations = {
  EN: {
    BUILD: 'Build',
...
export default class Translator implements ITranslator {
  ...
  public static current: Translator;
  public static initiliaze (local: RadishLocality = 'EN') {
    return new Translator(local);
  }

  protected local: RadishLocality = 'EN';

  public get TEXTS () {
    return TEXTS[this.local];
  }
  
  constructor (local: RadishLocality = 'EN') {
    this.local = local;
    Translator.current = this;
  }

  public changeLocality(local: RadishLocality = 'EN') {
    this.local = local;
  }
